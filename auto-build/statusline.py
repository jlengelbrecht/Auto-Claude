...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/auto-build/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .auto-build-status or auto-build/."""
    cwd = Path.cwd()

    # Check current directory
    if (cwd / ".auto-build-status").exists():
        return cwd
    if (cwd / "auto-build").exists():
        return cwd

    # Walk up to find project root
    for parent in cwd.parents:
        if (parent / ".auto-build-status").exists():
            return parent
