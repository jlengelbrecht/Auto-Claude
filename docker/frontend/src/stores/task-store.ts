...
/**
 * Start a task - uses REST API with fallback to Electron IPC
 * @param taskId - The task/spec ID
 * @param projectId - Optional project ID (required for REST API in web container)
 * @param options - Optional start options
 */
export async function startTask(taskId: string, projectId?: string, options?: { parallel?: boolean; workers?: number }): Promise<void> {
  const store = useTaskStore.getState();

  // Try to find projectId from store if not provided
  if (!projectId) {
    const task = store.tasks.find(t => t.id === taskId || t.specId === taskId);
    projectId = task?.projectId;
  }

  // Try REST API first (for web container)
  const token = getAuthToken();
  if (token && projectId) {
    try {
      const response = await fetch(`/api/projects/${projectId}/tasks/${taskId}/start`, {
        method: 'POST',
        headers: {
      ...
      console.error('Failed to start task via REST:', response.status);
    } catch (error) {
  ...
  if (window.electronAPI?.startTask) {
    window.electronAPI.startTask(taskId, options);
  }
}

/**
 * Stop a task - uses REST API with fallback to Electron IPC
 * @param taskId - The task/spec ID
 * @param projectId - Optional project ID (required for REST API in web container)
 */
export async function stopTask(taskId: string, projectId?: string): Promise<void> {
  const store = useTaskStore.getState();
  ...
  if (token && projectId) {
    try {
      const response = await fetch(`/api/projects/${projectId}/tasks/${taskId}/stop`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      console.error('Failed to stop task via REST:', response.status);
    } catch (error) {
      console.error('REST API error:', error);
    }
  }
