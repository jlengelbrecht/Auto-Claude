import { X, Folder, File, FileCode, FileJson, FileText, FileImage, Link, ExternalLink } from 'lucide-react';
 ...
 */
function getFileIcon(name: string, isDirectory: boolean, isRemote?: boolean): React.ReactNode {
  // Remote URLs get a link icon
  if (isRemote) {
    return <Link className="h-4 w-4 text-info shrink-0" />;
  }
      ...
      <div className={cn('space-y-2', className)}>
        ...
        <div className="space-y-1">
          {files.map((file) => (
            ...
            >
              {/* File/folder icon */}
              {getFileIcon(file.name, file.isDirectory, file.isRemote)}
